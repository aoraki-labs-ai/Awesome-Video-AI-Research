...
### 一次实跑:二十分钟,命中一个占 12.5% 的洞

一个电商图像团队按上述做法跑了第一次 diff(已脱敏),基准用的是从 850 件真实商品严格人评统计出的失败面分布:

| 失败桶 | 占比 | 已有检测器 | 结论 |
|---|---|---|---|
| logo/文字畸变 | 20.1% | OCR 逐字比对 | 已覆盖 |
| 图案/设计变化 | 11.4% | MP-LPIPS + SSIM-HF | 已覆盖 |
| 颜色偏移 | 8.1% | 掩码内 ΔE | 已覆盖 |
| **元素缺失** | **12.5%** | **无** | **没想到 → 盲区** |

"元素缺失"是**第二大失败桶**,而该团队的指标清单里**从头到尾没出现过这个维度**。更细的一层:已有的像素测量只能抓"少了一整件",抓不到"某个部件没画出来"——**这两件事在报表上长得一样,之前被当成了同一件。**

补法本来就现成(按部件掩码做逐部件在位检查),**难的从来不是补,是想到要补**。

> 这次实跑改变了该团队对完备性检查的优先级排序:原本排在校准之后,现在提前——**因为它成本极低(一次 diff 二十分钟),而且它找到的东西是校准永远找不到的。校准能告诉你 OCR 判得准不准,它永远不会告诉你"你没有任何检测器对着 12.5% 的失败"。**

### 三类枚举,失效方式各不相同(不是"多用几份更保险")

| 枚举类型 | 来源 | 覆盖什么 | **它自己的盲区** |
|---|---|---|---|
| **维度枚举** | 外部 benchmark(VBench-2.0 / MJ-Bench / VisionReward…) | 领域公认"该测什么" | **漏掉你这个领域特有的失败** |
| **失败面枚举** | 自己的人评统计(哪一类实际在坏、占比多少) | 已经发生过的失败 | **对新引入的机制盲**——没跑过的东西没有统计 |
| **机制枚举** | 自己的流水线拓扑(每个环节能出什么错) | 新组件引入的失败面 | 依赖你对自己系统的理解,漏掉跨环节耦合 |

前两类由实践总结,第三类是对第二类盲区的补丁:**每当流水线里加入一个新组件,失败面枚举对它的统计量为零、维度枚举写的时候也没考虑它**,只有沿着自己的架构逐环节问"这里能出什么错、我有没有检测器"才会触发。

### 防自欺:diff 本身会变成新的假闸门

一次 diff 只用了某一份枚举,就**只能声称"按这份枚举查过一遍",不能声称清单完备**。必须把**"用了哪几份枚举、各自查出什么"记进报表**,否则"做过完备性检查"这句话本身会变成另一个假闸门。

> 这与本篇 §2 的"不达标的 judge 只提示不拦截"是同一条防线:**一个流程动作声称的覆盖范围,必须能被审计到具体用了哪些输入。**

### ⚠️ 与 reward hacking 的连接:闸门集的盲区就是未来的攻击面

本篇 §2 建议过一条便宜路径——**把已有的客观闸门直接改造成 RL 的 reward**(OCR 既当过滤器又当 reward 是已被验证的做法)。这条路径有一个必须先满足的前置条件:

**一旦闸门变成 reward,闸门没覆盖的维度就变成了免费劣化的通道。** 模型会把 OCR 优化到满分,同时让 OCR 不检查的东西(元素缺失、图案变化)自由退化——**因为那里没有梯度阻止它**。

所以上面那个 12.5% 的盲区,如果在改造成 reward **之后**才被发现,它不会只是"少测了一块",而是"**reward hacking 精确地流向了那一块**"。

> **结论:完备性检查是"闸门 → reward"改造的前置步骤,不是并行任务。** 更一般地——**对闸门集做完备性检查,和对 reward 做 reward hacking 分析,在闸门即将变成 reward 时是同一件事**;区别只在于前者问"我漏了什么",后者问"如果有人要钻空子会往哪钻",而**答案是同一个集合**。
