...
上一节那条判据能挡住**假收敛**,挡不住**共同缺席**。

一个自然的反制是"独立验证只能来自实测"——**但这句话不完全成立**。实测的**结果**是独立的,**测什么**却已经被证据塑造了:会去量某个指标、会把某个环节前置,都是因为文献这么指。**耦合从"结论"层转移到了"议程"层。**

于是风险换了形态:**从"得到错的结论"变成"有共同的盲区"——那些双方都没想到要测的东西。** 而共同盲区**治不了靠更多交叉验证**,因为交叉验证只能检查已经想到的项。**一个两边都没提出的问题,不会以"收敛"的形式暴露,它根本不会出现在任何一份文档里。**

> 这与本篇 §2 的"只验证好样本、从没验证坏样本会被拦住"是同一个结构:**缺席的东西不产生信号。**

**这也说明"完备性检查"必须是独立一条纪律,不能并进"怀疑打分器"**——怀疑打分器管的是**已有指标准不准**(失败形态:错数字);完备性检查管的是**该有的指标有没有缺**(失败形态:没数字)。两者的发现方式完全不同。

### 这个领域自己就有过一次大规模共同盲区

**VBench 1.0 → 2.0 本身就是一次完备性修正。** 1.0 测的是"表面保真"——像素质量、基础 prompt 对齐;**物理与常识这两类根本不在指标体系里**,整个领域有好几年没有人系统地测它。等到 2.0 把它们加进来,配合 VideoPhy-2 一量,才发现**最好的模型在物理 hard 集上 joint 只有 22%**。

**盲区不是"测得不准",是"根本没在测",而且它掩盖的恰恰是最大的那个失败面。** 任何团队在设计自己的评价栈时都会重复这个模式——你只会给你想到的维度建检测器。

### 怎么让完备性检查不沦为形式

**空泛地问"我们漏了什么"几乎不产生信息**,因为它依赖内省,而内省对盲区天然无效。有效的做法是把它变成**一次机械的 diff**:

> 拿一份**你没在用**的外部枚举,逐条对照"这一维我有没有检测器",没有的记下来并给出理由(不需要的 / 需要但没做 / 没想到)。

可用作 diff 基准的现成枚举:**VBench-2.0 的 18 个细维**、**VideoPhy-2 的显式物理规则标注**、**MJ-Bench 的四视角**、**VisionReward 的 9 维 20 子维 64 题**、**VMBench 的五个运动指标**。它们的价值不在于你要全跑,而在于**它们是别人独立想出来的维度清单**——正好补上"你想不到自己想不到什么"这个洞。

把这条固定进迭代流程(建议与评测集刷新同频),不要靠谁偶然想起。

### 两条配套的操作判据

- **证据门槛按"它将排除多少选项"定,而不是按"它听起来多有道理"定。** 一条主张若成立会让某整类方案直接出局,它需要的证据强度远高于一条只影响参数选择的主张——影响面越大,越不能靠一句 intro 里的因果解释。
- **"更正与原文并存"要配一条生命周期约定,否则会反噬。** 它的副作用是文档变长且难读;约定为:**更正段落只保留到下一次结构性重写,重写时把演进过程移进修订记录、正文只留结论。** 否则三十条更正之后"可追溯"就变成了"不可读",等于白做。
